...
(function() {
    ...
    function setupListeners(target, onClose) {
        ...
        walkActionButton.on('mousedown', function actionCallback(){
            if (!!onClose) onClose();
            ...
            walkActionButton.off('mousedown', actionCallback);
        });
    }
    ...
    function renderFrame(target, renderCallback) {
        ...
        walkWrapper.css({
            'height': (height + WALK_PADDING) + 'px',
            'width': (width + WALK_PADDING) + 'px',

            'margin-left': -((width + WALK_PADDING) / 2) + 'px',
            'margin-top': -((height + WALK_PADDING) / 2) + 'px',
    ...
    function renderContent(target, renderCallback) {
        ...
        if (!itCanBeRenderedInRight) {
            ...
            textAlign = itCanBeRenderedInLeft ? 'left' : 'left';
